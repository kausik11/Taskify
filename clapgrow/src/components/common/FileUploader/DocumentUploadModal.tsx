...
const UploadModalContent = ({
	...
	const uploadFiles = async () => {
		...
		await Promise.allSettled(promises).then((res) => {
			const fulfilledPromises: PromiseFulfilledResult<File>[] = res.filter(
				(r) => r.status === "fulfilled",
			) as PromiseFulfilledResult<File>[];
			const rejectedPromises: PromiseRejectedResult[] = res.filter(
				(r) => r.status === "rejected",
	...
	return (
		<>
			...
			<div>
				{fileErrors.length ? (
					fileErrors?.map((e: any, index) => (
						<ErrorBanner error={e} key={index} />
					))
				) : (
					<FileDrop
						files={files}
			...
			<DialogFooter>
				{fileErrors.length > 0 && (
					<Button variant={"ghost"} onClick={resetFilesAndErrors}>
            Reset
					</Button>
				)}
				<Button
