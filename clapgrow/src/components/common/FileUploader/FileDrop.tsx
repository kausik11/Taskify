...
export const FileDrop = ({
	...
	const fileSizeValidator = (file: any) => {
		if (maxFileSize && file.size > maxFileSize * 1000000) {
			toast.error(
				`Uh Oh! ${file.name} exceeded the maximum file size required.`,
			);
			return {
				code: "size-too-large",
	...
	return (
		<div className="flex flex-col items-center gap-4">
			{(maxFiles === undefined || files.length < maxFiles) && (
				<div
					className={`flex justify-center items-center rounded-lg h-28 ${uploading ? "hidden" : "flex"} ${error ? "border-red-500 border-dashed" : "border-gray-500 border-dashed"} border`}
					{...getRootProps()}
					onClick={() => {}}
					{...props}
				>
					...
					<div className="flex flex-col items-center space-y-2 px-8">
						...
						<div className="flex space-x-4 items-center justify-center">
							<button
								{...getRootProps()}
								className="flex items-center space-x-2 text-sm bg-transparent border border-gray rounded-md px-4 py-2"
							>
		...
		<li className="flex items-center py-2 gap-4 w-full text-sm">
			<div className="pl-1">
				{previewURL ? (
					...
					<LucidFile size={16} className="h-6 w-6" />
				) : (
					<Link size={16} className="h-6 w-6" />
				)}
			</div>

			<span className="flex-1 truncate">
				{file.fileType === "file" ? file.name : file.fileURL}
			</span>
