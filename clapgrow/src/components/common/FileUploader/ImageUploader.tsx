...
export const ImageUploader = ({
	...
	const uploadImage = (files: File[]) => {
		if (files.length > 0) {
			updateDoc(doctype, docname, {
				...
				.catch(() => {
					toast.error(
						`There was an error while uploading the image. ${updateDocError ? (updateDocError.exception ?? updateDocError.httpStatusText) : null}`,
					);
				});
		}
	};
	...
	const deleteImage = () => {
		updateDoc(doctype, docname, {
			...
			.catch(() => {
				toast.error(
					`There was an error while deleting the image. ${updateDocError ? (updateDocError.exception ?? updateDocError.httpStatusText) : null}`,
				);
			});
	};
	...
	return (
		<div className="flex flex-col items-center gap-4">
			{file || defaultFile ? (
				<div
					className={`relative ${props.className} w-${boxSize} h-${boxSize} rounded-md`}
				>
					<div className="image-container group">
						{file ? (
							...
							<img
								src={defaultFile}
								className={`image w-${boxSize} h-${boxSize} object-cover rounded-${borderRadius} object-center rounded-2xl`}
								alt={defaultFile}
							/>
						)}
						<Button
				...
				<div
					className={cn(
						`w-${boxSize} h-${boxSize} rounded-md border border-dashed border-gray-400 flex items-center justify-center cursor-not-allowed`,
						props.className,
					)}
				>
					<SpinnerLoader />
				...
				<div
					className={`h-${boxSize} aspect-square rounded-md border border-dashed border-gray-400 flex items-center justify-center cursor-pointer ${props.className}`}
					onClick={onOpen}
				>
...
export const ImagePreview = ({
	...
	return (
		<img
			src={previewURL}
			alt={file.fileType === "file" ? file.name : "Image"}
			className={`object-cover rounded-md h-${size} w-${size} border-r-${borderRadius} ${props.className} rounded-2xl`}
		/>
	);
};
