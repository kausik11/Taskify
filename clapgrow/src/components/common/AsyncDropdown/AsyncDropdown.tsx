...
export const AsyncDropdown = ({
	...
	const pageLimit = useMemo(
		() => limit || getSystemDefault("link_field_results_limit") || 10,
		[limit],
	);
	...
	const { call: linkTitleCall } = useFrappePostCall(
		"clapgrow_workflow.clapgrow_workflow.api.link.get_link_title",
	);
	...
	const items = filterOption
		? data?.message
			.slice(0, 50)
			.filter((item) => filterOption(item, searchInput))
		: data?.message;

	const stateReducer = useCallback(
		(
			state: UseComboboxState<ResultItem>,
			actionAndChanges: UseComboboxStateChangeOptions<ResultItem>,
		) => {
	...
	/** If routing is available on eMotive, we will route the user to the corresponding page on click */
	const { isRoutingAvailable } = useMemo(() => {
		const routing = doctype in routingMap;
		return { isRoutingAvailable: routing };
	}, [doctype]);
		...
		<div className="relative w-full">
			<div>
				<div className="relative w-full group">
					<Input
						className="file:text-foreground placeholder:text-muted-foreground selection:bg-primary selection:text-primary-foreground dark:bg-input/30 border-input flex h-9 min-w-0 border bg-transparent px-3 py-1 text-base shadow-xs transition-[color,box-shadow] outline-none file:inline-flex file:h-7 file:border-0 file:bg-transparent file:text-sm file:font-medium disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50 md:text-sm focus-visible:ring-blue-700 focus-visible:ring-[0.1px] focus-visible:border-blue-700 aria-invalid:ring-red-500 aria-invalid:border-red-500 w-full pr-9 rounded-md hover:cursor-pointer"
					...
					{isLoading ? (
						<AsyncSpinnerLoader />
					) : isRoutingAvailable && field.value ? (
						<DocumentLink
							value={field.value}
							doctype={doctype}
  ...
  isOpen: boolean;
	...
	return (
		<div
			...
			style={{
				boxShadow:
          "0px 8px 14px rgba(25, 39, 52, 0.08), 0px 2px 6px rgba(25, 39, 52, 0.04)",
			}}
		>
			{children}
		</div>
...
const ErrorContainer = ({ error }: { error?: FrappeError }) => {
	if (error) {
		return (
			<div
				className="absolute w-full z-10 bg-white dark:bg-gray-800 shadow-lg border border-gray-200 rounded-b-md p-2"
				style={{
					boxShadow:
            "0px 8px 14px rgba(25, 39, 52, 0.08), 0px 2px 6px rgba(25, 39, 52, 0.04)",
				}}
...
const DropdownContainer = ({
  ...
  heightAdjust?: boolean;
	...
	return (
		<ul
			className={`absolute w-full shadow-lg border border-gray-200 rounded-md bg-white ${
				heightAdjust ? "max-h-[140px]" : "max-h-80"
	...
	return (
		<li
			className={`px-2 py-2 rounded-md cursor-pointer ${
				highlightedIndex === index ? "bg-gray-100" : "hover:bg-gray-200"
		...
		>
			<div className="flex flex-col gap-0">
				...
				<span className="text-xs">
					{(item.description ?? "").replace(htmlReplaceRegex, "")}
				</span>
			</div>
		</li>
	);
		...
		<div className="relative w-full">
			<div>
				<div className="relative w-full group">
					...
					{isLoading ? (
						<AsyncSpinnerLoader />
					) : isRoutingAvailable && selectedValue ? (
						<DocumentLink
							value={selectedValue}
							doctype={doctype}
...
export const DocumentLink = ({
	...
	if (isDisabled || isReadOnly || !value) return null;
	...
	return (
      ...
      hidden group-hover:flex group-focus-within:flex"
		>
			<Link
				to={getRoute(doctype as never, value)}
				target={clickOpenInNewTab ? "_blank" : undefined}
				rel={clickOpenInNewTab ? "noopener noreferrer" : undefined}
				tabIndex={-1}
